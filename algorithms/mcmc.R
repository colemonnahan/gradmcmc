...
leapfrog <- function(L, fn, gr, eps, init, r.cur, covar=NULL){
    ...
    theta.new <- theta.cur
    r.new <- r.cur
    theta.leapfrog <- matrix(NA, nrow=L, ncol=length(init))
    r.leapfrog <- matrix(NA, nrow=L, ncol=length(init))
    K <- P <- H<- rep(NA, length=L)
    logalpha <- rep(NA, length=L)
    ## Make a half step for first iteration
    for(i in 1:L){
        xx <- step(theta=theta.new, r=r.new, eps=eps)
        theta.new <- xx$theta
        r.new <- xx$r
        K[i] <- fn2(theta.new)
        P[i] <- sum(r.new^2)/2
        ...
        r.leapfrog[i,] <- r.new
        logalpha[i] <-
            -fn2(theta.cur)+fn2(theta.new)+ sum(r.cur^2)/2-sum(r.new^2)/2
    }
    if(!is.null(covar)) {
        theta.leapfrog <- t(apply(theta.leapfrog, 1, function(x) chd %*% x))
    }
    return(list(accept=pmin(1,exp(logalpha)),
                par=rbind(init, theta.leapfrog), L=L,
