...
## practice using STan to generate random data, then check that
## non-centering of the t distribution is working right
setwd('old')
N <- 1e6
fit <- stan(file='t_tests.stan', chains=1, iter=N, warmup=0)
xx <- data.frame(extract(fit, permuted=FALSE)[,1,])
par(mfrow=c(1,3))
qqplot(xx$z, rnorm(N, 8,3));abline(0,1)
qqplot(xx$t, rt(N, df=4)*3+8);abline(0,1)
qqplot(xx$t, xx$t2); abline(0,1)
setwd(main.dir)
